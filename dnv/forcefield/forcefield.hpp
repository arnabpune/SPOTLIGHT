...
/**This exception is thrown when the requested atom-type is not available in the list of loaded ones*/
class NoSuchAtomTypeException :  public exception {};
...
static Atom* (*WEIGHTED_ATOM_SELECTOR_FUNCTION)(const ForceField&,const std::vector<std::string>&,Atom*,const std::vector<Atom*>&,int devid)=nullptr;
...
public:
  ...
  std::vector<std::pair<pair_ss,pair_dd>> one_four_interactions; /**<Stores 1-4 interaction parameters. Not used in later versions of DeNovo. We use a fudge factor of 0.5*/
...
public:
	...
	*/
	void loadRules(const std::string& fl,bool emptyRules=true)
	{
		auto exrules=chemtools::getRuleset(fl,this);
		if(emptyRules) rules=exrules;
		else
		{
			for(auto rl : exrules) rules.push_back(rl);
		}
		cout << "Loaded rules. Rules for "<<rules.size()<<" atom types found\n";
	}
  ...
  /**@brief Convert string list to Atom list. Does NOT copy the Atom objects*/
  std::vector<Atom*> toAtomList(const std::vector<std::string>& lst) const
  {
	  std::vector<Atom*> ret;
	  for(const std::string& so : lst)
	  {
		  for(Atom* at : atom_types) {if(at->toString()==so) ret.push_back(at);}
	  }
	  return ret;
  }
  ...
  Atom* getAtom(const std::string& s,double x,double y,double z) const
  {
	...
	*/
	inline std::vector<std::string> listAtomsByRule(Atom* s,Molecule* m,Atom* temp=nullptr) const;
	...
	std::vector<std::string> listAtomsByRule(Atom* s,const std::vector<Atom*>& bnd,Atom* temp=nullptr) const
	{
		...
		try
		{
			...
			{
				#ifdef SCHUFFLE_RULES
				//return std::vector<Atom*>(1,randomSelect(allrules).suggest(s,this)); //Would return only one atom
				return randomSelect(allrules).getRuleGroup();
				#else
				return ruleset[ind].getRuleGroup();
				//return std::vector<Atom*>(1,ruleset[ind].suggest(s,this)); //Would return only one atom
				#endif
			}
		}
		catch(ElementNotFoundException& ex) {}
		std::vector<std::string> allowedatoms;
		...
		for(const BondData&  b: getAllowedBonds(s))
		{
			...
			if(sat) allowedatoms.push_back(ret->toString());
			delete ret;
		}
		...
		else
		{
			if(temp)
			{
				...
				//std::vector<std::string> refats;
				for(std::string a : allowedatoms) {if(a[0]==chk) f2.push_back(a); /*else refats.push_back(a);*/}
				if(f2.size()) allowedatoms=f2;
			}
			return allowedatoms;
		}
	}
	/*@brief Select a random atom from the list of possible bondable atoms (See listAtomsByRule(Atom*,Molecule*,Atom*))*/
	inline Atom* selectAtomByRule(Atom* s,Molecule* m,Atom* temp=nullptr,int devid=0,Atom* (*selector)(const ForceField&,const std::vector<std::string>&,Atom*,const std::vector<Atom*>&,int) = WEIGHTED_ATOM_SELECTOR_FUNCTION) const;
	...
	Atom* selectAtomByRule(Atom* s,const std::vector<Atom*>& bnd,Atom* temp=nullptr,int devid=0,Atom* (*selector)(const ForceField&,const std::vector<std::string>&,Atom*,const std::vector<Atom*>&,int) = WEIGHTED_ATOM_SELECTOR_FUNCTION) const
	{
		auto allowedatoms=listAtomsByRule(s,bnd,temp);
		if(!selector)
		{
			std::string r=randomSelect(allowedatoms);
			//for(Atom*& ap : allowedatoms) {if(ap!=r) delete ap;}
			return this->getAtom(r);
		}
		return this->getAtom(selector(*this,allowedatoms,s,bnd,devid)->toString());
	}
  ...
  /**@brief Post-hoc removal of a particular atom type (and related) parameters. It is not recommended you do this unless you are really sure.*/
  void removeAtomType(const std::string& atype,bool removeParams=true,bool editRules=false);
  ...
  const std::pair<double,double>& getImproperParameters(const std::vector<std::string>& ats) const
  {
	...
	{
		std::vector<std::string> ats; ats.push_back(a1->toString()); ats.push_back(a2->toString()); ats.push_back(a3->toString()); ats.push_back(a4->toString());
...
namespace chemtools
{
	/**@brief Decide how many double bonds an atom requires based on hybridization and element*/
	static inline int getNumUnsat(Atom* el,ForceField* ff=nullptr)
	{
		char elname=el->toString()[0];
		switch(elname)
		{
			case 'C':
				if(ff && ff->hasCategory("halogen") && ff->getCategory("halogen").contains(el)) return 0;
				else return 4-el->getStandardValency();
			case 'N': return 3-el->getStandardValency();
			case 'O': return 2-el->getStandardValency();
			case 'P': return 5-el->getStandardValency();
			case 'S':
				if(el->getStandardValency()==1) return 1;
				if(el->getStandardValency()==2) return 0;
				else if(el->getStandardValency()==3) return 1;
				else return 2;
			default:
				return 0;
		}
...
void ForceField::removeAtomType(const std::string& atype,bool removeParams,bool editRules)
{
	std::string search=stringfx::trim(atype);
	//TODO: Check that atom type exists
	int aidx=-1;
	for(int i=0;i<this->atom_types.size();i++)
	{
		if(this->atom_types[i]->toString()==search) {aidx=i; break;}
	}
	if(aidx==-1) throw NoSuchAtomTypeException();
	if(editRules)
	{
		int ridx=-1;
		for(int i=0;i<rules.size();i++)
		{
			if(get<0>(rules[i])==search) ridx=i;
			for(auto& rl : get<1>(rules[i]))
			{
				auto& grp=rl.getRuleGroup();
				std::vector<int> delidxs;
				for(int j=0;j<grp.size();j++) {if(grp[j]==search) delidxs.push_back(j);}
				for(int j=delidxs.size()-1;j>=0;j--) grp.erase(grp.begin()+delidxs[j]);
			}
		}

		//Remove rules for this atom type
		if(ridx!=-1) rules.erase(rules.begin()+ridx);
	}
	if(removeParams)
	{
		std::vector<int> remidxs;
		for(int i=0;i<length_parameters.size();i++)
		{
			if(length_parameters[i].contains(search)) remidxs.push_back(i);
		}
		int shift=0;
		for(int i=remidxs.size()-1;i>=0;i--) length_parameters.erase(length_parameters.begin()+remidxs[i]);
	}

	//Removing from bond mapping
	int mapidx=-1;
	for(int i=0;i<atombondmap.size();i++)
	{
		if(get<0>(atombondmap[i])==search) {mapidx=i; continue;}
		else
		{
			std::vector<BondData>& bondings=get<1>(atombondmap[i]);
			for(int j=0;j<bondings.size();j++)
			{
				if(bondings[j].contains(search))
				{
					bondings.erase(bondings.begin()+j);
					break;
				}
			}
		}
	}
	if(mapidx!=-1) atombondmap.erase(atombondmap.begin()+mapidx);

	//Finally remove the atom type
	this->atom_types.erase(this->atom_types.begin()+aidx);
...
public:
	...
	~Topology() {for(Atom* atype : atomtypes) delete atype;}
...
public:
	...
	inline int getNumAngles() const {return angles.size();}
	inline int getNumDihedrals() const {return dihedrals.size();}
	inline int getNumImpropers() const {return impropers.size();}

	inline std::pair<std::pair<int,int>,std::pair<double,double>> getBondParametersAt(int idx) const {return make_pair(bonds[idx],bondpars[idx]);}
	inline std::pair<std::vector<int>,std::vector<double>> getAngleParametersAt(int idx) const {return make_pair(angles[idx],anglepars[idx]);}
	inline std::pair<std::vector<int>,std::vector<double>> getDihedralParametersAt(int idx) const {return make_pair(dihedrals[idx],dihpars[idx]);}
	inline std::pair<std::vector<int>,std::pair<double,double>> getImproperParametersAt(int idx) const {return make_pair(impropers[idx],improppars[idx]);}
