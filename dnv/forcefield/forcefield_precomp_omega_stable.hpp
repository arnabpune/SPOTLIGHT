...
public:
	...
	Residue(const std::string& rnm,const std::vector<std::string>& lines)
	{
		...
		for(const std::string& s : lines)
		{
			stringstream ss(s);
			ss >> nm >> rl;
			ids.push_back(nm); actual.push_back(rl);
		}
	}
...
public:
	...
	*/
	...
	inline bool contains(Atom* a) const {return contains(a->toString());}
...
public:
  ...
  ForceField(const std::string& ff_file) :ForceField()
  {
	...
	/**@brief Load categories from an external file into this forcefield. Categories can also be loaded when the ForceField object is created (See: ForceField(const std::string&,const std::string&))*/
	void loadCategories(const std::string& catf)
	{
		categories.empty();
	...
	/**@brief Load rules from a given file. See the atom-type definition section of documentation PDF for more details on format*/
	void loadRules(const std::string& fl) {rules=chemtools::getRuleset(fl,this); cout << "Loaded rules. Rules for "<<rules.size()<<" atom types found\n";}
  ...
  Atom* getAtom(const std::string& s,double x,double y,double z) const
  {
	...
	*/
	Atom* selectRandomBondableAtom(Atom* a,Atom* bias=nullptr) const
	{
		...
		if(!allatoms.size()) NoBondsAvailableException();
		if(!biasedatoms.size()) return getAtom(randomSelect(allatoms));
		else return getAtom(randomSelect(biasedatoms));
	}
	/**@brief Situationally choose an atom that can bond to the selected atom
		 @details Get a copy of one of the loaded atom-types such that it can bond to the source atom (see params).<br/>
	...
	*/
	...
	Atom* selectAtomByRule(Atom* s,const std::vector<Atom*>& bnd,Atom* temp=nullptr) const
	{
		...
		try
		{
			...
			if(ind!=-1) return ruleset[ind].suggest(s,this);
		}
		...
		else
		{
			if(temp)
			{
				...
				std::vector<Atom*> f2;
				for(Atom* a : allowedatoms) {if(a->toString()[0]==chk) f2.push_back(a);}
  ...
  void loadLengths(const std::string& s);
	/**@brief Load bond-angle data from a given file*/
  void loadAngles(const std::string& s);
	/**@brief Load dihedral data from a given file*/
  void loadDihedrals(const std::string& s);
  ...
  void loadBondParameters(std::string lengthfn=LengthDataFile,std::string anglefn=AngleDataFile,std::string dihedralfn=DihedralDataFile,std::string improperfn=ImproperDataFile)
  {
    //cout << lengthfn << "; Now working\n";
    loadLengths(lengthfn);
    loadAngles(anglefn);
    loadDihedrals(dihedralfn);
  ...
  const std::pair<double,double>& getImproperParameters(const std::vector<std::string>& ats) const
  {
	...
	/**@brief Look for the given atom-set combination in the improper database. If not found, return default values based on hybridization*/
	const std::pair<double,double> getImproperParametersFailsafe(const std::vector<std::string>& ats,int nb,int mv) const
	{
		...
		catch(DataNotAvailableException ex)
		{
			...
			if(nb==2) return make_pair(0,480);
			else return make_pair(0.59,45);
		}
	}
  ...
  {
    double th=acos((((a0->seek_x())-(a1->seek_x()))*((a2->seek_x())-(a1->seek_x()))+((a0->seek_y())-(a1->seek_y()))*((a2->seek_y())-(a1->seek_y()))+((a0->seek_z())-(a1->seek_z()))*((a2->seek_z())-(a1->seek_z())))/(sqrt(pow(((a0->seek_x())-(a1->seek_x())),2)+pow(((a0->seek_y())-(a1->seek_y())),2)+pow(((a0->seek_z())-(a1->seek_z())),2))*sqrt(pow(((a2->seek_x())-(a1->seek_x())),2)+pow(((a2->seek_y())-(a1->seek_y())),2)+pow(((a2->seek_z())-(a1->seek_z())),2))));
  ...
  double calculateAngleEnergy(const std::string& s1,const std::string& s2,const std::string& s3,double th,double ub,bool rigid=false,double acut=THETACUT) const
  {
    ...
    else
    {
      ...
      double angle_energy=(par[0]-th)*(par[0]-th)*par[1]/2;
      double urey_bradley_energy=(par[2]-ub)*(par[2]-ub)*par[3]/2;
  ...
  double calculateDihedralEnergy(const std::string& s1,const std::string& s2,const std::string& s3,const std::string& s4,double ph,bool rigid=false,double acut=DIHEDTHETACUT) const
  {
    ...
    else return par[1]*(1+cos(par[2]*ph-par[0]));
  }
  ...
  {
    double *m=cross_product(a1->seek_x()-a0->seek_x(),a1->seek_y()-a0->seek_y(),a1->seek_z()-a0->seek_z(),a2->seek_x()-a0->seek_x(),a2->seek_y()-a0->seek_y(),a2->seek_z()-a0->seek_z()),*n=cross_product(a2->seek_x()-a0->seek_x(),a2->seek_y()-a0->seek_y(),a2->seek_z()-a0->seek_z(),a3->seek_x()-a0->seek_x(),a3->seek_y()-a0->seek_y(),a3->seek_z()-a0->seek_z()),q=acos(dot_product(m[0],m[1],m[2],n[0],n[1],n[2])/(sqrt(pow(m[0],2)+pow(m[1],2)+pow(m[2],2))*sqrt(pow(n[0],2)+pow(n[1],2)+pow(n[2],2))));
    delete[] m; delete[] n;
...
void ForceField::loadLengths(const std::string& filename)
{
	cout << "Loading lengths from: "<<filename<<"\n";
	//length_parameters=std::vector<std::pair<pair_ss,pair_dd>>();
	length_parameters=std::vector<BondData>();
	...
	while(1)
	{
		...
		length_parameters.push_back(BondData(first,second,b0,kb));
...
void ForceField::loadAngles(const std::string& filename)
{
	cout << "Loading angles from: "<<filename<<"\n";
	angle_parameters=std::vector<std::pair<vector_s,vector_d>>();
	ifstream f;
	f.open(filename,ios::in);
...
void ForceField::loadDihedrals(const std::string& filename)
{
	cout << "Loading dihedrals from: "<<filename<<"\n";
	dihedral_parameters=std::vector<std::pair<vector_s,vector_d>>();
	ifstream f;
	f.open(filename,ios::in);
...
/**
	...
	<p>This namespace makes use of the Eigen libraries. See Eigen: http://eigen.tuxfamily.org/ for more details. (git repo: https://gitlab.com/libeigen/eigen.git)</p>
*/
namespace forcefx
{
...
/**\class Topology forcefield.hpp "forcefield/forcefield.hpp"
	This is the topology class. It stores all the parameters required to reconstruct the molecule's topology.<br/>It can be used internally by DeNovo, and can be exported to TOP format (GROMACS topology format).
