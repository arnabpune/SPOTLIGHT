...
public:
  ...
  Atom* getAtom(const std::string& s,double x,double y,double z) const
  {
	...
	*/
	...
	std::vector<Atom*> listAtomsByRule(Atom* s,const std::vector<Atom*>& bnd,Atom* temp=nullptr) const
	{
		...
		try
		{
			...
			if(ind!=-1) return std::vector<Atom*>(1,ruleset[ind].suggest(s,this));
		}
	...
	*/
	...
	inline std::pair<MolecularFragment*,std::pair<Atom*,std::vector<Atom*>>> selectFragmentByRule(Atom* s,const std::vector<Atom*>& bnd,const FragmentSet& fs,Atom* temp=nullptr) const {return randomSelect(listFragmentsByRule(s,bnd,fs,temp));}
