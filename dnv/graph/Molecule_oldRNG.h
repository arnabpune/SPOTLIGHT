...
//Some constants
static const int DEFAULT_MARX=5,DEFAULT_MARY=5,DEFAULT_MARZ=5,DEFAULT_RES=10,LEVELLIMIT=20;
...
template<class T> const T& weighedSelect(const std::vector<T>& objects,const std::vector<double>& cumu_weights)
{
  double rv=throwarandompoint(0,1.00)*cumu_weights[cumu_weights.size()-1];
...
template<class T> std::pair<T,double>& weighedSelectBoltzmann(std::vector<std::pair<T,double>>& list,const double& t)
{
	std::pair<T,double>& r=list[0];
	for (size_t i = 1; i < list.size(); i++)
	{
		if(throwarandompoint(0,1)<boltzmannFactor(get<1>(list[i])-get<1>(r),t)) r=list[i];
	}
	return r;
}
template<class T,class U> std::pair<T,std::pair<double,U>>& weighedSelectBoltzmannFirst(std::vector<std::pair<T,std::pair<double,U>>>& list,const double& t)
{
	std::pair<T,std::pair<double,U>>& r=list[0];
	for (size_t i = 1; i < list.size(); i++)
	{
		if(throwarandompoint(0,1)<boltzmannFactor(get<0>(get<1>(list[i]))-get<0>(get<1>(r)),t)) r=list[i];
...
template<class T,class U> std::pair<T,std::pair<U,double>>& weighedSelectBoltzmannSecond(std::vector<std::pair<T,std::pair<U,double>>>& list,const double& t)
{
	...
	for (size_t i = 1; i < list.size(); i++)
	{
		...
		if(throwarandompoint(0,1)<boltzmannFactor(get<1>(get<1>(list[i]))-get<1>(get<1>(r)),t)) r=list[i];
    //if(throwarandompoint(0,1)<boltzmannFactor(get<iv>(get<1>(list[i]))-get<iv>(get<1>(r)),t)) r=list[i];
	}
	return r;
}
...
namespace chemtools
{
	...
	*/
  static std::pair<Eigen::Vector3d,std::pair<double,double>> getAllowedPositions(Atom* src,Molecule* mol,Atom* natom,const System& ff,int num=10,int tries=10,bool restr=false,const Category* aromcat=nullptr);
...
public:
	...
	*/
	...
	/**@brief Load a molecule from TXT format (see algorithm PDF for details on TXT format)
		 @details This is the Molecule class constructor that can load molecules from TXT format file. Recent DeNovo versions support PDB and GRO formats, however, this method is kept for backward support
...
public:
  ...
  bool attempt(const System& s,Atom* gP,Atom* nat,Atom*& nxt,int trials=10,bool restr=false,int nps=0);
	...
	*/
  bool reduce(const System& s,int trials=15,bool forced=false); //Acceptable
  ...
  bool completeAttempt(ostream& logfile,const System& s,Atom* src,int trials=25,int nupper=90,int level=0,int levellim=LEVELLIMIT,bool restr=false,bool superseed=false,bool reseed=false,bool deseed=false,Molecule*& common_newroot=NULLMOL,int common_seedlength=0)
  {
    ...
    if(!nat) {std::cout << "Warning: no atom found\n"; if(logfile) logfile << "No atom to bond to: "<<src->toString()<<"\n"; return false;}
		if(nat->seek_valency()>=2) {if(throwarandompoint(0,1)<CYCPROB || (src->mustCycl && !src->isCyclized()) || (s.ff->hasCategory("ring") && s.ff->getCategory("ring").contains(nat))) nat->mustCycl=true;}
		//ADD Cyclization to atom-types here
    cout << "Trial: "<<nat->toString()<<"\t";
    logfile << "Trial: "<<nat->toString()<<"\t";
    Atom* nxt=nullptr;
    if(!attempt(s,src,nat,nxt,trials,restr)) {delete nat; return false;}
    bondAtom(nat,src);
    if(nxt)
    {
      cout << nat->toString()<< "-"<<nxt->toString()<<"\n";
    ...
    while(!s.ff->isSatisfied(nat,getBondedAtoms(nat)))
    {
      ...
      if(!completeAttempt(logfile,s,nat,trials,nupper,level+1,levellim,restr,superseed,reseed,deseed,common_newroot,common_seedlength)) return false;
    }
  ...
  /*bool completeAtom(const System& s,Atom* src,int trials=15,nupper=90)
  {
	...
	*/
  void samplegrow(ostream& logfile,const System& s,int n,int trials=15,int nupper=90,int levellim=LEVELLIMIT,bool restr=false,bool superseed=false,bool reseed=false,bool deseed=false,Molecule*& common_newroot=NULLMOL,int common_seedlength=0)
  {
		#ifdef REALTIME
    ...
    while(getEffectiveSize()<n)
    {
      ...
      if(completeAttempt(logfile,s,gP,ATTEMPTTRIALS,nupper,0,levellim,restr,superseed,reseed,deseed,common_newroot,common_seedlength))
			{
  ...
  void dumpMol(const std::string& fn,const ForceField* ff=nullptr) const
  {
	...
	void dumpMol(ofstream& f,const ForceField* ff=nullptr) const//assume open
	{
		...
		const Category* halo=(ff)?&(ff->getCategory("halogen")):nullptr;
		for(int i=0;i<atoms.size();i++)
		{
			a=atoms[i];
      if(!a) continue;
	...
	*/
	inline void dumpMolAsTxt(ofstream& f) const {f<<atoms.size()<<"\n"; for(int i=0;i<atoms.size();i++) f << *(atoms[i]) << "\n";}
	/** Same as dumpMolAsTxt(const std::string&). Uses an output stream instead of filename. */
	inline void dumpMolAsTxt(const std::string& f) const {ofstream of; of.open(f,ios::out); this->dumpMolAsTxt(of); of.close();}
	...
	*/
	void makePruneFragment(int n)
	{
		...
		for(int i=0;i<atoms.size();i++)
		{
			...
			if(terminal) {terms.push_back(i); hydats.push_back(hyd); termconns.push_back(termholder);}
		}
		if(!terms.size())
		{
			#ifndef QUIET
				cout << "WARN: Prune fragment generation failed. No terminals to prune\n";
				describeStructure();
			#endif
		}
		cout << "Terminals:\n"; for(int i=0;i<terms.size();i++) cout<<terms[i]<<":"<< atoms[terms[i]]->toString()<<"\n";
		int rS=(int)throwarandompoint(0,terms.size());
...
Molecule::Molecule(const char *file,bool eval) : Molecule()
{
	ifstream f;
	...
	//atoms=new Atom*[N];
	int i=0;
	for(;i<N;++i)//copying atom co-ordinates and other information from file
	{
		int v;
		double x,y,z,wt,c,s,e,r;
		char *id=new char[8];
		f>>id>>x>>y>>z>>v>>wt>>c>>s>>e>>r;
		atoms.push_back(new Atom(id,x/10.0,y/10.0,z/10.0,v,wt,c,s,e,r));
		delete[] id;
	}
...
Molecule::Molecule(const std::string& infile,const ForceField& ff,const std::string& format,bool eval,bool warnf,char chain)
{
  ...
  while(true)
  {
    ...
    else if(format=="gro")
    {
      ...
      atnm=stringfx::trim(line.substr(11,4));
			rnam=stringfx::trim(line.substr(5,4));
  ...
  //N=atoms.size();
	#ifndef QUIET
  cout << N<<" atoms read into protein under format: "<<format<<"\n";
	#endif
  if(eval)
	{
...
public:
  ...
  Protein(const std::string& infile,const ForceField& ff,std::vector<int> hotspotreses,const std::string& format="pdb") : Molecule(infile,ff,format,false)
  {
	...
	*/
	...
	/**@brief After calculating the non-bonded energy (see Protein::calculateNonBondingEnergy(Molecule*)), the hotspot energy can be got through this function*/
  ...
  std::vector<System> generateLigands(const ForceField& ff,int n,int sizemin,const std::string& logfilename="dnv_protgen.log",std::vector<Molecule*> rootmols=std::vector<Molecule*>(),double rmax=2,double ecut=1,int osccount=5,int tottries=250,int tries_per_lig=45,double temp=298,bool restr=false,bool superseed=false,bool reseed=false,bool deseed=false,const quickgeom::Container* contbox=nullptr,Molecule* initmol=nullptr,bool prune=false,int prunedepth=3)
  {
    ...
    //int rv;
    	...
    	while(ret.size()<n)
	    {
		      ...
		      testm=new Molecule(chosenroot);
					testm->type=0;
					if(prune)
						testm->makePruneFragment((int)throwarandompoint(0,prunedepth)+1);
		      ...
		      s1.addMolecule(testm);
					...
					logfile << testm->getEffectiveSize() << " is effective size of testm to begin with\n";
		  			    if(prune) testm->samplegrow(logfile,s1,sizemin+rootsize,tries_per_lig,maxsize,LEVELLIMIT,restr,superseed,reseed,deseed,common_newroot,common_seedlength);
		  			    else testm->samplegrow(logfile,s1,sizemin,tries_per_lig,maxsize,LEVELLIMIT,restr,superseed,reseed,deseed,common_newroot,common_seedlength);
		      ...
		      K[id]=0;
				...
				if(smilehandle)
				{
					...
					lockcoll=true;
					#ifndef FFCHARGES
					while(netcomm::queueSize())
					{
						logfile << "Queue size: "<< netcomm::queueSize() << "\n"; logfile.flush();
						if(ret.size()>n+5) {netcomm::drainQueue(); break;}
						s1=netcomm::processRequest(logfile);
						testm=s1.getDrugMolecule();
					#endif
						if(!testm) {lockcoll=false; continue;}
			      if(coll.size())
			      {
							e1=testm->binding_energy()+testm->pE; e2=coll[coll.size()-1].getDrugMolecule()->binding_energy()+coll[coll.size()-1].getDrugMolecule()->pE;
	...
	*/
	Molecule* quickPrune(const ForceField& ff,Molecule* p,std::string logfilename="dnv_prune.log",int iters=100,int expand=0,int tries_per_lig=45,double temp=298,int prunedepth=3)
	{
		int id=0;
		Molecule* curmol=p;
		if(!p) return p;
		...
		#endif
		{
			...
			tempm->samplegrow(logfile,s1,p->getEffectiveSize()+expand,tries_per_lig,p->getEffectiveSize()+expand+5,LEVELLIMIT);
...
{
	ofstream f; f.open(of,fstream::out);
	for(Molecule* m : mols)
	{
...
bool Molecule::attempt(const System& s,Atom* gP,Atom* nat,Atom*& nxt,int trials,bool restr,int nps)
{
	#ifndef QUIET
		cout << trials << " ";
	#endif
	...
	try
	{
		if(s.ff->hasCategory("aromatic")) penv=chemtools::getAllowedPositions(gP,this,nat,s,20,10,restr,&(s.ff->getCategory("aromatic")));
		else penv=chemtools::getAllowedPositions(gP,this,nat,s,20,10,restr);
	...
	else
	{
		...
		#endif
		if(trials>0 && nps<4) {return attempt(s,gP,nat,nxt,trials-1,restr,nps+1);} //Trial 'trial' number of times, but don't waste time if No position is available (thrice)
		else return false;
	}
}
...
static std::pair<Eigen::Vector3d,std::pair<double,double>> chemtools::getAllowedPositions(Atom* src,Molecule* mol,Atom* natom,const System& sys,int num,int tries,bool restr,const Category* aromcat)
{
  ...
  if(!bondcol.size()) {angex=false; temp=quickgeom::getRandomVectorsInShell(Eigen::Vector3d(0,0,0),length,num*20);}
  ...
  if(angex)
  {
    ...
    else
    {
      ...
      //dih.push_back(sys.ff->getDihedralParametersFailsafe(rs2->toString(),get<0>(rs)->toString(),src->toString(),natom->toString())[0]);
			if(aromcat && aromcat->contains(src) && !src->isCyclized()) {cout << "Forcing 0 dihedral for aromatic atom\n"; dih=std::vector<double>(1,0.0);}
			else
			{
  ...
  //cout << "Required angle: "<<ang<<"\n";
    ...
    if(angex)
    {
      if(!dihexist) {/*cout << "No dihedral constraints\n";*/ temp=quickgeom::getConePositions(fixedbond,length,ang,num*10);} //Checked that angle is in radians.
      else
      {
        //cout << "\ttemp = "<<"quickgeom::getArcPositions("<<fixedbond.transpose()<<","<<defbond.transpose()<<","<<length<<","<<ang<<","<<dih<<","<<num<<");\n";
        temp=quickgeom::getArcPositions(fixedbond,defbond,length,ang,dih,num);
      }
    }
		for(auto& v : temp)
		{
  ...
  if(ret.size())
	{
		auto& elr=weighedSelectBoltzmannFirst(ret,sys.temp);
		#ifndef QUIET
			cout << "\nTrialled\n";
...
bool Molecule::reduce(const System& s,int trials,bool forced) //Acceptable
{
	cout << "Reduction begins\n";
	Atom *ha=s.ff->getAtom("H"),*nxt,*tmp;
	int h=0;
	std::vector<Atom*> chn,chn2;
	double div,ebe,ehbe;
	bool nx=true,fx=false;
	int sz=atoms.size();
	...
	for(int I=0;I<sz;I++)
	{
		...
		if(!ta || !ta->isHydrogen())
		{
				...
				cout << "!H\n";
			#endif
			if(ta) delete ta; continue;
		}
		int m=a->seek_valency();
		div=(2*PI)/m;
		nx=true;
		for(int i=0;i<m;i++)
		{
			std::vector<Eigen::Vector3d> tposes=quickgeom::getArcPositions(bnd,(fx)?fixed:quickgeom::getRandomVectorInShell(Eigen::Vector3d(0,0,0),1),0.09,((a->getStandardValency()==4)?109.5:120)*(PI/180.0),ForceField::expandMultiplicity(div*i,2),60);
			for(Eigen::Vector3d& v : tposes)
			{
				nx=true;
				ta->setPosition(a->getPosition()+v);
				for(Atom* x : chn) {if(ta->distanceFrom(x)<RCUT/1.5) {nx=false; break;}}
				if(nx)
				{
					bondAtom(ta,a);
			...
			{
				#ifndef QUIET
					cout << "Could not reduce with required dihedral:"<<div*i<<"\t: "<<a->toString()<<"\n";
				#endif
				break;
			}
		}
		...
		/*while(true)
		{
			if(a) cout <<"\t"<< a->toString()<<"\n";
			else break;
			Atom* ta=s.ff->selectAtomByRule(a,getBondedAtoms(a));
			if(!ta->isHydrogen()) {delete ta; break;}
			if(!attempt(s,a,ta,nxt,trials)) {cout << "WARN: Failed to reduce\n"; delete ta;; break;}
			else {bondAtom(ta,a); h++;}
			if(!a->canBond()) break;
		}*/
	}
...
namespace netcomm
{
	...
	*/
	...
	//static volatile Queue<MolRequest> molq;
	...
	inline System processRequest(std::ostream& logfile)
	{
