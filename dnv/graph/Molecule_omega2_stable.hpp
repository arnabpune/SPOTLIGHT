...
#include <unistd.h>
	...
	#ifndef FORCE_CHARGES
		#define FORCE_CHARGES 1
	#endif
#endif
...
namespace chemtools
{
	...
	static inline double getNonbondingPotential(Atom* a1,Atom* a2,double r)
  {
	...
	/**@brief */
	static std::vector<std::pair<std::vector<Eigen::Vector3d>,double>> getAllAllowedFragmentPositions(Atom* src,Molecule* mol,MolecularFragment* frag,int handle,const System& ff,int num=10,int tries=10,bool restr=false,const Category* aromcat=nullptr,int devid=0);
}
namespace molfx
{
	#ifndef NOJSON
...
public:
  ...
  void samplegrow(ostream& logfile,const System& s,int n,int trials=15,int nupper=90,int levellim=LEVELLIMIT,bool restr=false,bool superseed=false,bool reseed=false,bool deseed=false,Molecule*& common_newroot=NULLMOL,int common_seedlength=0,int devid=0)
  {
    ...
    try
		{
			...
			#ifdef FORCE_CHARGES
				this->uniformChargeAdjustments(this->getNetCharge(*s.ff));
			#endif
		}
	...
	*/
	void fragmentgrow(ostream& logfile,const System& s,const FragmentSet& frags,int n,int trials=25,int nupper=120,int levellim=LEVELLIMIT,bool restr=false,int devid=0)
	{
  ...
  void dumpMol(const std::string& fn,const ForceField* ff=nullptr) const
  {
	...
	void dumpMol(ofstream& f,const ForceField* ff=nullptr) const//assume open
	{
		...
		#ifdef VARIEDCOUNT
			f << "REMARK "<<HAC<<" 'heavy' atoms counted while running with varied counting scheme\n";
		#endif
...
public:
	...
	*/
	void dropSingleAtom(int din)
	{
		...
		#ifdef VARIEDCOUNT
			 if(VCLIST->contains(atoms[din])) HAC--;
		#endif
	...
	*/
	void makePruneFragment(int n,int devid=0)
	{
		...
		HC-=hydats[rS].size();
		#ifdef VARIEDCOUNT
			 if(VCLIST->contains(atoms[terms[rS]])) HAC--;
		#endif
...
public:
	MolecularFragment(std::istream& f,bool eval,bool cpt,const ForceField* ff=nullptr,const std::vector<int>& handles=std::vector<int>()) : Molecule(f,eval,cpt,ff)
	{
		hands=handles;
...
public:
	...
	std::pair<Atom*,std::vector<Atom*>> getHandleAtom(Atom* ref=nullptr)
	{
		...
		else
		{
			...
			for(int i=0;i<hands.size();i++)
			{
				if(atoms[hands[i]]->toString()==ref->toString()) {al.push_back(hands[i]); spts.push_back(supports[i]);}
			}
			if(al.size())
			{
  ...
  while(true)
  {
    ...
    try
    {
      ...
      atnm=rsd.getActual(atnm);
		...
		#ifdef VARIEDCOUNT
			 if(VCLIST->contains(atoms[atoms.size()-1])) HAC++;
		#endif
...
public:
  ...
  std::vector<System> generateLigands(const ForceField& ff,int n,int sizemin,const std::string& logfilename="dnv_protgen.log",std::vector<Molecule*> rootmols=std::vector<Molecule*>(),double rmax=2,double ecut=1,int osccount=5,int tottries=250,int tries_per_lig=45,double temp=298,bool restr=false,bool superseed=false,bool reseed=false,bool deseed=false,const quickgeom::Container* contbox=nullptr,Molecule* initmol=nullptr,bool prune=false,int prunedepth=3)
	#else
	std::vector<System> generateLigands(const ForceField& ff,const FragmentSet& frags,int n,int sizemin,const std::string& logfilename="dnv_protgen.log",std::vector<Molecule*> rootmols=std::vector<Molecule*>(),double rmax=2,double ecut=1,int osccount=5,int tottries=250,int tries_per_lig=45,double temp=298,bool restr=false,bool superseed=false,bool reseed=false,bool deseed=false,const quickgeom::Container* contbox=nullptr,bool prune=false)
	#endif
  {
		//Setup
		envprot=this;
  ...
  if(angex)
  {
    ...
    else
    {
      ...
      //dih.push_back(sys.ff->getDihedralParametersFailsafe(rs2->toString(),get<0>(rs)->toString(),src->toString(),natom->toString())[0]);
				...
				if(!natom->isHydrogen() && aromcat && aromcat->contains(src) && !src->isCyclized() && !rs2->isCyclized())
			#else
...
static std::vector<std::pair<std::vector<Eigen::Vector3d>,double>> chemtools::getAllAllowedFragmentPositions(Atom* src,Molecule* mol,MolecularFragment* frag,int handle,const System& sys,int num,int tries,bool restr,const Category* aromcat,int devid) //num is num per each atom trialled. So num**3 trials per residue
{
	std::vector<std::pair<std::vector<Eigen::Vector3d>,double>> ret;
	Molecule* core=mol;
...
inline std::vector<Atom*> ForceField::listAtomsByRule(Atom* a,Molecule* m,Atom* exa) const {return listAtomsByRule(a,m->getBondedAtoms(a),exa);}
...
{
	std::vector<Atom*> alats=listAtomsByRule(src,bnd,exa);
