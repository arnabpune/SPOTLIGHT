...
{
  class ZMQIOThread;
  static std::vector<ZMQIOThread*> zmqthreads;
  class ZMQIOThread
  {
  ...
  public:
    ...
    ~ZMQIOThread() {socket.close(); context.close();}
  ...
  /**@brief Send a message over socket (as a string) an receive the reply*/
  static std::string communicateWithSocket(zmq::socket_t& socket,const std::string& data,std::ostream& dout=std::cout)
  {
  ...
  static void initThreads(std::ostream& logs,int n=1)
  {
    zmqthreads=std::vector<ZMQIOThread*>();
    logs << "Initializing ZMQ IO threads\n";
    for(int i=0;i<n;i++) zmqthreads.push_back(new ZMQIOThread(logs));
    logs << "Done initializing\n";
  }
  static void stopThreads(std::ostream& logs)
  {
    for(int i=0;i<zmqthreads.size();i++) delete zmqthreads[i];
  }
  inline static zmq::socket_t& getSocket(int devid=0) {return zmqthreads[devid]->getSocket();}
