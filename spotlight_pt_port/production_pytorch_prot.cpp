...
int main(int argc,char** argv)
{
  ...
  while(getline(loadf,line))
  {
    ...
    else if(idtoken=="PruneMol") INP.prunemolfile=stringfx::trim(tokens[1]);
 ...
 Molecule* templatemol=(argc>9)?new Molecule(INP.prunemolfile,ff,"pdb",true):nullptr;
 for(int J : INP.size)
 {
    int K=0;
    std::vector<System> mols=p.generateLigands(ff,INP.num,J,INP.progname+"_"+to_string(J)+".log",seeds,INP.seednum,false,INP.spread,1,INP.osc,300,50,298,"seeds_"+INP.progname+"_"+to_string(J)+".pdb",(INP.flagcodes[0]!='0'),(INP.flagcodes[1]!='0'),(INP.flagcodes[2]!='0'),(INP.flagcodes[3]!='0'),region,templatemol,false);
    for(System& s : mols)
    {
