...
int main(int argc,char** argv)
{
  ...
  for(int i=0;i<MAX_THREADS_OMP;i++)
  {
  	std::ofstream* of=new std::ofstream(); of->open("dnv_prod_"+std::to_string(i)+".log",ios::out);
	ofs.push_back(of);
  }
#else
  	std::ofstream* of=new std::ofstream(); of->open("dnv_prod.log",ios::out);
	ofs.push_back(of);
#endif
  ...
  #endif
  {
    ...
    try {m->samplegrow(*(ofs[tid]),sys,esize,40,100,20,false,false,false,false,NULLMOL,0,tid);}
#else
    molenvmt->replaceMolecule(m);
    try {m->samplegrow(*(ofs[tid]),sys,esize,40,100,20);}
#endif
    ...
    {
      cout << root->toString()<<"\t"; cout << "Failed!";
	...
	netcomm::processRequest(*(ofs[tid]),m,tid);
	if(!m)
    {
  ...
  #endif
  for(int i=0;i<ofs.size();i++) ofs[i]->close();
